...
class CandidateQueueManager:
    ...
    Notes:
    - step(...) returns:
        snapshot_before: candidates that existed before current update
        ready_after:     candidates that are considered stable after update
    - if drop_reported=True, candidates that were already reported via on_report(...)
      will not be returned by ready(...) again.
    """
    ...
    def expire(self, now: float) -> None:
        ...
        for cid in dead:
            ...
            self._reported_ids.discard(cid)

    def on_report(self, cand_ids: List[int]) -> None:
        """
        Mark candidates as already reported.
        Relevant only when drop_reported=True.
        """
        if not self.drop_reported:
            return

        for cid in cand_ids:
            if cid in self._candidates:
                self._reported_ids.add(int(cid))
    ...
    def _bbox_iou(b1, b2) -> float:
        ...
        inter_wh = np.maximum(
            0.0,
            np.minimum(b1[2:], b2[2:]) - np.maximum(b1[:2], b2[:2]),
        )
    ...
    def ready(self, now: float) -> List[Candidate]:
        ...
        for c in self._candidates.values():
            ...
            if self.drop_reported and c.cand_id in self._reported_ids:
                continue
